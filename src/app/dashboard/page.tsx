...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const verifyRole = async () => {
      ...
      if (currentRole && currentRole !== 'engineer' && currentRole !== 'admin') {
        router.push('/client');
      }
    };
  ...
  useEffect(() => {
    ...
    return () => { unsubscribeProjects(); unsubscribeClients(); };
  }, [user]);

  useEffect(() => {
    if (!selectedProject) return;

    const q = query(collection(db, 'feedback'), where('projectId', '==', selectedProject.id));
  ...
  const saveProjectContext = async (projectId: string) => {
    try {
      await updateDoc(doc(db, 'projects', projectId), { technicalContext: projectContext });
      addToast('Technical context saved!');
  ...
  const assignClient = async (projectId: string) => {
    ...
    try {
      ...
      if (querySnapshot.empty) {
        addToast('No client found with that email. Ensure they registered as a client.', 'error');
        return;
      }
      ...
      await updateDoc(projectRef, { assignedClients: [...currentClients, clientId] });
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#050505]">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-neutral-600 text-sm font-medium">Loading your dashboard...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#050505] flex text-white">
      ...
      {/* Sidebar */}
      <aside className="w-72 bg-[#0D0D0D] border-r border-[#1A1A1A] flex flex-col shrink-0">
        <div className="p-8">
        ...
        <div className="mt-auto p-8 border-t border-[#1A1A1A]">
          <div className="flex items-center gap-3 mb-6 p-3 bg-[#1A1A1A] rounded-2xl border border-[#242424]">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[10px] text-neutral-500 uppercase font-bold tracking-widest">Engineer · Admin</p>
            </div>
          </div>
          <button
          ...
          {activeTab === 'projects' && (
            <>
              <header className="flex items-center justify-between mb-10">
                <div>
                  <h1 className="text-4xl font-display font-bold text-white mb-1">Projects</h1>
                  <p className="text-neutral-500">Manage repositories and process client feedback</p>
                </div>
                <button
              ...
              {projects.length === 0 ? (
                <div className="py-24 text-center bg-[#0D0D0D] border border-dashed border-[#1F1F1F] rounded-[2rem] mb-10">
                  ...
                  <p className="text-neutral-500 font-bold text-xl mb-2">No projects yet</p>
                  <p className="text-neutral-600">Click "Add Project" to connect a GitHub repository</p>
                </div>
              ) : (
                    ...
                    >
                      <div className="flex items-center gap-3 mb-4">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {project.language && (
                            <span className="text-[10px] text-neutral-600 font-bold uppercase tracking-widest">{project.language}</span>
                          )}
                        </div>
                      </div>
                      <p className="text-sm text-neutral-500 mb-5 line-clamp-2 leading-relaxed">{project.description}</p>
                      <div className="flex items-center justify-between">
                        <span className="text-[#00A388] bg-[#00A388]/10 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest">Active</span>
                        <div className="flex items-center gap-3">
                          <button
                            ...
                            className="flex items-center gap-1 text-[10px] font-bold uppercase tracking-widest text-neutral-600 hover:text-white transition-colors"
                          >
                            <Settings className="w-3 h-3" /> Context
                          </button>
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                  ...
                  >
                    <div className="flex items-center justify-between border-b border-[#1A1A1A] pb-6">
                      <h2 className="text-2xl font-display font-bold text-white">
                        Feedback · <span className="text-[#00A388]">{selectedProject.name}</span>
                      </h2>
                      <div className="flex items-center gap-3">
                        <StatusBadge icon={<CheckCircle2 className="w-4 h-4" />} label={`${resolvedCount} Resolved`} color="emerald" />
                        <StatusBadge icon={<AlertCircle className="w-4 h-4" />} label={`${pendingCount} Open`} color="amber" />
                      </div>
                    </div>

                    <div className="space-y-5">
                      {feedbacks.map((feedback) => (
                        <div key={feedback.id} className="bg-[#0D0D0D] border border-[#1A1A1A] rounded-[2rem] overflow-hidden hover:border-[#00A388]/20 transition-all">
                          <div className="p-8">
                            <div className="flex items-start justify-between mb-5">
                              <div className="flex items-center gap-3">
                                <div className="w-10 h-10 bg-[#1A1A1A] rounded-full flex items-center justify-center border border-[#242424] text-sm font-bold text-[#00A388]">
                                  {(feedback.clientName || 'C')[0].toUpperCase()}
                                </div>
                                <div>
                                  <p className="font-bold text-white">{feedback.clientName || 'Client'}</p>
                                  <p className="text-xs text-neutral-600">{new Date(feedback.createdAt).toLocaleString()}</p>
                                </div>
                              </div>
                              <div className="flex items-center gap-3">
                                <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest ${
                                  feedback.type === 'bug' ? 'bg-red-500/10 text-red-500' : 'bg-[#00A388]/10 text-[#00A388]'
                                }`}>
                                  {feedback.type}
                                </span>
                                <select
                            ...
                            <p className="text-neutral-300 leading-relaxed mb-6">{feedback.content}</p>
                            ...
                            {!feedback.aiSuggestion ? (
                              ...
                              >
                                <div className="flex items-center gap-2 mb-4 text-[#00A388]">
                                  ...
                                  <span className="font-bold text-xs uppercase tracking-[0.15em]">AI Suggestion & Fix</span>
                                  {feedback.analyzedAt && (
                                    <span className="ml-auto text-[10px] text-neutral-700 font-medium">
                                      {new Date(feedback.analyzedAt).toLocaleDateString()}
                                    </span>
                                  )}
                                </div>
                                <div className="prose prose-invert prose-sm max-w-none prose-pre:bg-[#0D0D0D] prose-pre:border prose-pre:border-[#1A1A1A] prose-headings:text-[#00A388] prose-code:text-[#00A388]">
                                  <ReactMarkdown>{feedback.aiSuggestion}</ReactMarkdown>
                                </div>
                              </motion.div>
                            )}
                          </div>
                      ...
                      {feedbacks.length === 0 && (
                        <div className="py-24 text-center bg-[#0D0D0D] border border-dashed border-[#1A1A1A] rounded-[2rem]">
                          ...
                          <p className="text-neutral-700 mt-1 text-sm">Assign this project to a client to start receiving feedback</p>
                        </div>
                      )}
                    </div>
                  </motion.section>
          ...
          {activeTab === 'clients' && (
            <>
              <header className="mb-10">
                ...
                <p className="text-neutral-500">All registered clients on the platform</p>
              </header>
                ...
                {allClients.map((client) => (
                  ...
                  >
                    <div className="flex items-center gap-4 mb-5">
                      ...
                      <div>
                        <h3 className="font-bold text-white">{client.email?.split('@')[0]}</h3>
                    ...
                    <div className="space-y-2.5 text-xs font-bold uppercase tracking-widest">
                      <div className="flex justify-between text-neutral-600">
                        <span>Role</span><span className="text-[#00A388]">Client</span>
                      </div>
                      <div className="flex justify-between text-neutral-600">
                        <span>Joined</span><span>{new Date(client.createdAt).toLocaleDateString()}</span>
                      </div>
                    </div>
                  </motion.div>
                ))}
      ...
      <AnimatePresence>
        {showProjectSettings && (
          <Modal onClose={() => setShowProjectSettings(null)}>
            <h3 className="text-2xl font-display font-bold mb-2">Technical Context</h3>
            <p className="text-neutral-500 text-sm mb-6">Describe your tech stack and architecture to help AI give better suggestions.</p>
            <textarea
              value={projectContext}
              onChange={(e) => setProjectContext(e.target.value)}
              placeholder="e.g. Next.js 15 app using Firebase + Tailwind. Main logic in /lib/api.ts..."
              className="w-full h-40 p-4 bg-[#1A1A1A] border border-[#2A2A2A] text-white rounded-2xl outline-none focus:ring-2 focus:ring-[#00A388] mb-6 resize-none text-sm"
            />
            <div className="flex gap-3">
              <button onClick={() => setShowProjectSettings(null)} className="flex-1 py-3 bg-[#1A1A1A] text-neutral-400 rounded-xl font-bold hover:bg-[#242424] transition-all">Cancel</button>
              <button onClick={() => saveProjectContext(showProjectSettings)} className="flex-1 py-3 bg-[#00A388] text-white rounded-xl font-bold hover:bg-[#008F76] transition-all">Save</button>
            </div>
          </Modal>
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {showClientAssign && (
          <Modal onClose={() => setShowClientAssign(null)}>
            <h3 className="text-2xl font-display font-bold mb-2">Assign Client</h3>
            <p className="text-neutral-500 text-sm mb-6">Enter the client's registered email to grant access to this project.</p>
            <input
              type="email"
              placeholder="client@example.com"
              value={clientEmail}
              onChange={(e) => setClientEmail(e.target.value)}
              onKeyDown={(e) => e.key === 'Enter' && assignClient(showClientAssign)}
            ...
            />
            <div className="flex gap-3">
              <button onClick={() => setShowClientAssign(null)} className="flex-1 py-3 bg-[#1A1A1A] text-neutral-400 rounded-xl font-bold hover:bg-[#242424] transition-all">Cancel</button>
              <button
              ...
              >
                {assigning ? <Loader2 className="w-5 h-5 animate-spin" /> : 'Assign'}
              </button>
            </div>
          </Modal>
        )}
...
function NavItem({ icon, label, active = false, onClick }: { icon: React.ReactNode; label: string; active?: boolean; onClick?: () => void }) {
  return (
    <button
      onClick={onClick}
      className={`w-full flex items-center gap-4 px-4 py-3 rounded-xl font-bold transition-all text-sm ${
        active ? 'bg-[#00A388] text-white shadow-lg shadow-[#00A388]/20' : 'text-neutral-600 hover:bg-[#1A1A1A] hover:text-neutral-300'
      }`}
    >
      {icon}
      {label}
...
function StatusBadge({ icon, label, color }: { icon: React.ReactNode; label: string; color: 'emerald' | 'amber' }) {
  const styles = {
    emerald: 'text-emerald-500 bg-emerald-500/10 border-emerald-500/20',
    amber: 'text-amber-500 bg-amber-500/10 border-amber-500/20',
  };
  return (
    <span className={`flex items-center gap-2 px-3 py-1.5 rounded-full border text-[10px] font-bold uppercase tracking-widest ${styles[color]}`}>
      {icon} {label}
    </span>
  );
}
